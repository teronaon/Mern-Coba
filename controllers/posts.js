...
export const getMahasiswa = async(req,res)=>{
	try{
		...
		for (var mahasiswa of Mahasiswas) {
			if (mahasiswa.Gambar.data != undefined) {
...
export const createMahasiswa = async(req,res)=>{
	...
	if (req.file == undefined) {
		return res.status(400).json({messsage : "Gambar cannot empty"});
	}
	var obj = {
        Nama: req.body.Nama,
        NIM: req.body.NIM,
	...
	const cekId = await Jurusans.findById(obj.Jurusan);
